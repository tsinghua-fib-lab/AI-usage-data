...
model = BertForSequenceClassification.from_pretrained(os.path.join(model_dir,'finetune_abstract',f'epoch{model_index}'))
tokenizer = BertTokenizer.from_pretrained(os.path.join(model_dir,'finetune_abstract',f'epoch{model_index}'))
model.to(device)
model.eval()
...
if os.path.exists(os.path.join('..','results','Dict_ClassifyWork_Abstract_Raw')):
    ClassifyWork_Abstract_Raw=pickle.load(open(os.path.join('..','results','Dict_ClassifyWork_Abstract_Raw'),'rb'))
    print(len(ClassifyWork_Abstract_Raw))
else:
    ClassifyWork_Abstract_Raw=dict()
...
for chunk in tqdm(works):
    for _,line in chunk.iterrows():
        ...
        if id in ClassifyWork_Abstract_Raw:
            continue
        ...
        if len(abstract_batch)==batch_size:
            ...
            for j in range(batch_size):
                ClassifyWork_Abstract_Raw[id_batch[j]]=output[j]
    ...
    chunk_count+=1
    if chunk_count%50==0:
        pickle.dump(ClassifyWork_Abstract_Raw,open(os.path.join('..','results','Dict_ClassifyWork_Abstract_Raw'),'wb'))
