...
model = BertForSequenceClassification.from_pretrained(os.path.join(model_dir,'finetune_title_extend1',f'epoch{model_index}'))
tokenizer = BertTokenizer.from_pretrained(os.path.join(model_dir,'finetune_title_extend1',f'epoch{model_index}'))
model.to(device)
model.eval()


if os.path.exists(os.path.join('..','results','Dict_ClassifyWork_Title_Extend1')):
    ClassifyWork_Title_Extend1=pickle.load(open(os.path.join('..','results','Dict_ClassifyWork_Title_Extend1'),'rb'))
    print(len(ClassifyWork_Title_Extend1))
else:
    ClassifyWork_Title_Extend1=dict()
...
for chunk in tqdm(works):
    for _,line in chunk.iterrows():
        ...
        if id in ClassifyWork_Title_Extend1:
            continue
        ...
        if len(title_batch)==batch_size:
            ...
            for j in range(batch_size):
                ClassifyWork_Title_Extend1[id_batch[j]]=output[j]
    ...
    chunk_count+=1
    if chunk_count%50==0:
        pickle.dump(ClassifyWork_Title_Extend1,open(os.path.join('..','results','Dict_ClassifyWork_Title_Extend1'),'wb'))
